...
echo "[STEP 1/9] Configuring Tsinghua University mirror..."
echo "Backing up original apt sources..."
BACKUP_FILE="/etc/apt/sources.list.d/ubuntu.sources.bak.$(date +%Y%m%d%H%M%S)"
if [ -f "/etc/apt/sources.list.d/ubuntu.sources" ]; then
    cp /etc/apt/sources.list.d/ubuntu.sources "$BACKUP_FILE"
    echo "Original file backed up to $BACKUP_FILE"
else
    echo "No existing sources file found, creating new"
fi
echo "[COMPLETE] Source backup completed"
...
Signed-By: /usr/share/keyrings/ubuntu-archive-keyring.gpg
EOF
echo "[COMPLETE] Mirror configuration updated"

# Update system packages
echo "[STEP 2/9] Updating system packages..."
apt update && apt -y upgrade
echo "[COMPLETE] System packages updated"

# Install required components
echo "[STEP 3/9] Installing desktop components..."
apt install -y ubuntu-desktop xwayland
echo "[COMPLETE] Desktop components installed"

# Create WSLG fix service
echo "[STEP 4/9] Configuring WSLG fix service..."
...
echo "[STEP 5/9] Modifying user runtime service..."
...
echo "[STEP 6/9] Reloading systemd daemon..."
systemctl daemon-reload
echo "[COMPLETE] Daemon reloaded"

# Enable and start services
echo "[STEP 7/9] Starting WSLG services..."
...
echo "[STEP 8/9] Configuring Xorg replacement..."
...
echo "[9/9] Creating display configuration..."
mkdir -p ~/.config
